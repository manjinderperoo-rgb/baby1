...
# === New Database for Channel Verification ===
def init_db():
    """Initialize SQLite database for channel verification"""
...
def is_authorized_user(user_id: int) -> bool:
    """Check if user is authorized (either individually or via group)"""
...
# === NEW: Check if command is mentioned in group ===
def is_command_mentioned(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    """Check if bot is mentioned in group commands"""
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Save channel member if in private chat
    if chat_type == Chat.PRIVATE:
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if update.effective_chat.type != Chat.PRIVATE and not is_command_mentioned(update, context):
        return
...
async def setig(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if num < 0 or num >= len(data['accounts']):
        await reply_with_mention(update, "⚠️ Invalid number. ⚠️")
        return
    data['default'] = num
    save_user_data(user_id, data)
    acc = data['accounts'][num]['ig_username']
    await reply_with_mention(update, f"✅ {num+1}. {acc} now is your default account. ⭐")
